...
$query1 = "SELECT DISTINCT pjk FROM kamus ORDER BY pjk ASC";
$result1 = mysqli_query($conn, $query1);

// Retrieve selected values
$selectedColumn1 = isset($_POST['pjk']) ? $_POST['pjk'] : '';
$selectedColumn2 = isset($_POST['pelaksana']) ? $_POST['pelaksana'] : '';
$selectedColumn3 = isset($_POST['kegiatan']) ? $_POST['kegiatan'] : '';
$selectedColumn4 = isset($_POST['uraian']) ? $_POST['uraian'] : '';
$selectedColumn5 = isset($_POST['satuan']) ? $_POST['satuan'] : '';

if (isset($_POST['submit'])) {
    // Fetch options for second column based on selected value from column1
    $query2 = "SELECT DISTINCT pelaksana FROM kamus WHERE pjk = '$selectedColumn1' ORDER BY pelaksana ASC";
    $result2 = mysqli_query($conn, $query2);

    // Fetch options for third column based on selected values from column1 and column2
    $query3 = "SELECT DISTINCT kegiatan FROM kamus WHERE pjk = '$selectedColumn1' AND pelaksana = '$selectedColumn2' ORDER BY kegiatan ASC";
    $result3 = mysqli_query($conn, $query3);

    // Fetch options for third column based on selected values from column1 and column2 and column3
    $query4 = "SELECT DISTINCT uraian FROM kamus WHERE pjk = '$selectedColumn1' AND pelaksana = '$selectedColumn2' AND kegiatan = '$selectedColumn3' ORDER BY uraian ASC";
    ...
    if (!empty($selectedColumn2)) {
        $whereCondition .= " AND pelaksana = '$selectedColumn2'";
    }
    if (!empty($selectedColumn3)) {
        $whereCondition .= " AND kegiatan = '$selectedColumn3'";
    }
    if (!empty($selectedColumn4)) {
        $whereCondition .= " AND uraian = '$selectedColumn4'";
    }
...
<body class="index-page">
  <div class="wrapper">
    ...
    <div class="container">
      ...
      <h3>Hitung Angka Kredit Anda melalui Filter</h3>
        <form method="post">
          <div class="row">
            <div class="col-md-12">
          ...
          <div class="row">
            <div class="col-md-12">
              <label for="pelaksana">Jabatan Anda, Statistisi:</label>
              <select name="pelaksana" id="pelaksana">
                <option value="">Statistisi:</option>
                <?php
                    while ($row = mysqli_fetch_assoc($result2)) {
                        $option = $row['pelaksana'];
              ...
              <select name="uraian" id="uraian">
                ...
                <?php
                      while ($row = mysqli_fetch_assoc($result4)) {
                          $option = $row['uraian'];
                          echo "<option value='$option'" . ($selectedColumn4 == $option ? ' selected' : '') . ">$option</option>";
                      }
                ?>
              </select>
                  <input type="submit" name="submit" value="Pilih" class="btn btn-sm btn-default">
              ...
              if (isset($_POST['submit'])) {
                  if (mysqli_num_rows($resultResult) > 0) {
                      ...
                      while ($rowResult = mysqli_fetch_assoc($resultResult)) {
                          ...
                          $tableRows .= "<td>$column2Value pada Kegiatan <b>$selectedColumn3</b></td>";
                          $tableRows .= "<td><center>$column3Value</center></td>";
                      ...
                      if (isset($volume) && $volume != '') {
                        ...
                        echo '<th><center>Satuan hasil</center></th>';
