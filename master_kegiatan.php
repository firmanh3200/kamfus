				...
				<div class="row">
					<div class="col-md-12 col-lg-12 col-xl-12">
						<section class="panel">
							...
							<h3><strong><center>Master Kegiatan KamFuS</center></strong></h3>
								<hr>
								<table class="table table-bordered table-striped mb-none" id="sosial" data-swf-path="https://cdn.datatables.net/tabletools/2.2.4/swf/copy_csv_xls_pdf.swf">
									...
									<tbody>
                    					<?php
										$qry = $conn->query("SELECT * FROM kamus order by pjk,kegiatan asc");
                    					while($row= $qry->fetch_assoc()):
                    						$trans = get_html_translation_table(HTML_ENTITIES,ENT_QUOTES);
                    					?>
                    					<tr>
                    						<td><?php echo ucwords($row['pjk']) ?></td>
                    						<td><?php echo ucwords($row['kegiatan']) ?></td>
                    						<td><?php echo ucwords($row['uraian'])?></td>
											<td>
												<a class="dropdown-item" href="edit_kegiatan.php&id=<?php echo $row['id'] ?>">Update Isian</a>
											</div>
										</tr>	
                    				<?php endwhile; ?>
                    				</tbody>
